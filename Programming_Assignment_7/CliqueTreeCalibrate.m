...
function [P, logZ] = CliqueTreeCalibrate(P, isMax)
    ...
    % logZ, the log of the partition function.
     if (doLogZ)
        %%% YOUR CODE HERE:
        % Note LALS: The Partition Function Z of the Joint Distribution could be found
        % from any cluster belief of the calibrated clique Tree using unnormalized messages...
        % essentially a belief is a product of variable elimination.
        %for i = 1:N
         for i = 1:1
        % Iterate through cliques and find the final potentials
           betaI=P.cliqueList(i);
           for j = 1:N
               % Iterate through messages and multiply them       
                betaI = FactorProduct(betaI, unnormalizedMessages(j,i));         
           end
            i
           %betaI.val;
            ZI=sum(betaI.val)
