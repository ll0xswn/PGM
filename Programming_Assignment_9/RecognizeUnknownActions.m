...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  for i=1:nActionLabels
       ...
       % InitClusterProb= (1/KClass).*ones(size(datasetTrain(i).InitialClassProb));
       %  [d1 d2 InitClassProb] = EM_cluster(datasetTrain(i).poseData, G, InitClusterProb, 20);
         ...
         KClass=6;
       % InitClassProb = (1/KClass).*ones(size(datasetTrain(i).InitialClassProb,1),KClass);
         % [d1 d2 InitClassProb] = EM_cluster(datasetTrain(i).poseData, G, InitClassProb, 10);

        %InitClassProb = rand(size(datasetTrain(i).InitialClassProb,1),KClass);
       InitClassProb = zeros(size(datasetTrain(i).InitialClassProb,1),KClass);
        for jj=1:size(InitClassProb,1)
     %       rand("seed",jj);

            rind=randperm(KClass);
         
            InitClassProb(jj,rind(1))=1;
      %      InitClassProb(jj,rind(2:end))= (1 - 0.7)/(KClass-1);
            InitClassProb(jj,:)=  InitClassProb(jj,:)./sum( InitClassProb(jj,:));         
         end

       %   rind=randperm(size(datasetTrain(i).InitialClassProb,1));
       %   InitClassProb= InitClassProb(rind,:);

        % [d1 d2 InitClassProb] = EM_cluster(datasetTrain(i).poseData, G, InitClassProb, 10);
      
         InitPairProb = 1/(KClass^2).*ones(size(datasetTrain(i).InitialPairProb,1), KClass^2);
         %InitPairProb = datasetTrain(i).InitialPairProb;

         [P loglikelihood ClassProb PairProb] = EM_HMM(datasetTrain(i).actionData, datasetTrain(i).poseData, G,  InitClassProb, InitPairProb, maxIter);
         P_A{i}=P;
