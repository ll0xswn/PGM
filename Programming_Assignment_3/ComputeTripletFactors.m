...
% Your code here: 
 nTripletList = length(tripletList);
 tripletValIndexes = zeros(nTripletList,1);
 tripletValues = zeros(nTripletList,1);

 % A vectorized way for the first loop below is:
 % tripletValIndexes=AssignmentToIndex(vertcat(tripletList.chars), [K K K]);
 % tripletValues= vertCat(triletList.factorVal);

 for j=1:nTripletList
    tripletValIndexes(j)=AssignmentToIndex(tripletList(j).chars, [K K K]);
    tripletValues(j)=tripletList(j).factorVal;
 end
...
for i=1:(n-2)
   ...
   factors(i).val(tripletValIndexes)=tripletValues;
 end
