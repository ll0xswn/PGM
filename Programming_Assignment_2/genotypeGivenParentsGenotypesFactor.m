...
IndA=IndexToAssignment([1:length(genotypeFactor.val)],genotypeFactor.card);

C = IndA(:, 1);
C_Al= genotypesToAlleles(C,:);

P1GN=IndA(:,2);
P1_Al = genotypesToAlleles(P1GN,:);

P2GN=IndA(:,3);
P2_Al = genotypesToAlleles(P2GN,:);
...
%ParentOne, Parent2 Alleles
    P1A=P1_Al(i,:);
    P2A=P2_Al(i,:);

%Possible Alleles Combinations for Parent Genotypes for Child Genotype i
    Pairs = [ P1A(1) P2A(1); P1A(1) P2A(2); P1A(2) P2A(1); P1A(2) P2A(2)];
    ReplCA=repmat(CA, size(Pairs,1), 1);
...
% Heterozygous Genotypes
    if (CA (1) != CA(2) )
       CA1=[ CA(2), CA(1)];
       ReplCA1=repmat(CA1, size(Pairs,1), 1);
       TM2 = (ReplCA1 == Pairs);
       nCA = nCA + sum(TM2(:,1).*TM2(:,2));
    end
    PCA= nCA/4.0;
    genotypeFactor.val(i) = PCA;
 end
