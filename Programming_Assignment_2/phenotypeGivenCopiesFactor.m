...
GenotypesAsAlleles = IndxA(1:end, 2:end);
PhenoType1 = GenotypesAsAlleles(1:2:end,:);

 k = 1;
 for i=1:size(PhenoType1,1)
    indGenoTypeI= allelesToGenotypes(PhenoType1(i,1), PhenoType1(i,2));
    phenotypeFactor.val(k)=alphaList(indGenoTypeI);
    phenotypeFactor.val(k+1)=1 - alphaList(indGenoTypeI);
    k=k+2;
  end
