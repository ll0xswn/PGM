...
nAlleles = length(alleleWeights{1});
nGenes=length(geneCopyVarOneList);
%assume ony 2 genes
phenotypeFactor.card= [2,nAlleles,nAlleles,nAlleles,nAlleles];


phenotypeFactor.val = zeros(1, prod(phenotypeFactor.card));
% Replace the zeros in phentoypeFactor.val with the correct values.

 IndxA=IndexToAssignment([1:length(phenotypeFactor.val)],phenotypeFactor.card);

 GenotypesAsAlleles = IndxA(1:2:end, 2:end);

 s = zeros(1, prod(phenotypeFactor.card)/2);
 for  i=1:nAlleles

   S1 = alleleWeights{1}(i).*((GenotypesAsAlleles(:,1) == i) + (GenotypesAsAlleles(:,3) == i));
   S2 = alleleWeights{2}(i).*((GenotypesAsAlleles(:,2) == i) + (GenotypesAsAlleles(:,4) == i));
   s = s + S1' + S2';
  end

sigArr = computeSigmoid(s);

phenotypeFactor.val(1:2:end)=sigArr;
phenotypeFactor.val(2:2:end)=1 - sigArr;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
